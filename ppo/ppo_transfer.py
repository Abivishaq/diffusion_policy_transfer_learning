...
class Args:
    ...
    """path to the source task checkpoint to transfer from"""
    freeze_layers: Optional[List[str]] = None
    """list of layers to freeze during transfer learning (e.g., ['actor.0', 'critic.0'])"""
    reinit_layers: Optional[List[str]] = None
    """list of layers to reinitialize for the target task"""
    transfer_mode: str = "full"
    """transfer mode: 'full' (all weights), 'partial' (specified layers), or 'features_only' (freeze encoder)"""
...
### 2. Add transfer learning setup function
def setup_transfer_learning(agent: Agent, args: Args) -> None:
    """Setup agent for transfer learning based on specified arguments"""
    if args.source_checkpoint:
        ...
        if args.transfer_mode == "full":
            ...
            current_state_dict = agent.state_dict()
            filtered_state_dict = {k: v for k, v in source_state_dict.items() 
                                 if any(k.startswith(prefix) for prefix in ['actor.0', 'actor.1', 'critic.0', 'critic.1'])}
            current_state_dict.update(filtered_state_dict)
            agent.load_state_dict(current_state_dict)
        ...
        if args.freeze_layers:
            for name, param in agent.named_parameters():
                if any(layer in name for layer in args.freeze_layers):
...
if __name__ == "__main__":
    ...
    if args.source_checkpoint:
        setup_transfer_learning(agent, args)
    elif args.checkpoint:
        # Regular checkpoint loading for continued training
        agent.load_state_dict(torch.load(args.checkpoint))

    ### Update optimizer to only train non-frozen parameters
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, agent.parameters()), 
