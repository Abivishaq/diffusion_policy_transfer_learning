...
class Agent(nn.Module):
    def __init__(self, env, args):
        ...
        # print(f"Observation dimension: {np.prod(env.single_observation_space.shape)}")
        self.condition_dim = 84#np.prod(env.single_observation_space.shape)


        self.noise_pred_net = ConditionalUnet1D(
            input_dim=self.act_dim, # act_horizon is not used (U-Net doesn't care)
            global_cond_dim=self.condition_dim, #np.prod(env.single_observation_space.shape), # obs_horizon * obs_dim
...
if __name__ == "__main__":
    ...
    for iteration, data_batch in enumerate(train_dataloader):
        ...
        # obs_seq
        obs_seq = data_batch['observations'] # (B, horizon, obs_dim)
        # print(f"obs_seq.shape: {obs_seq.shape}, action_seq.shape: {data_batch['actions'].shape}")
        # If obs_seq's obs_dim is less than 84, we need to pad it to 84
        if obs_seq.shape[-1] < 42:
            obs_seq = F.pad(obs_seq, (0, 42 - obs_seq.shape[-1]), value=0) 
